...
instance Monad (Parser t) where
  (Parser px) >>= f =
    Parser  (px >=> (\ (x, ts') -> runParser (f x) ts'))
...
parseExp s = case runParserEnd pExp s of
  ...
  [e] -> Right e
  _  -> Left "Input can be parsed in multiple ways"

example :: Either String Exp
...
(x |-> e') (ELet v e b) | x == v              = ELet v ((x |-> e') e) b
(x |-> e') (ELet v e b) | v `notElem` free e' = ELet v ((x |-> e') e) ((x |-> e') b)
(x |-> e') (ELet v e b)                       = let y' = fresh [b, e'] in
                        ELet y' ((x |-> e') e) ((x |-> e') ((v |-> EVar y') b))

-- ELet derivation:
-- (x |-> e') (ELet v ev f)
-- (x |-> e') (EApp (ELam v f) ev) -- EApp ((x |-> e') e1) ((x |-> e') e2)
-- EApp ((x |-> e') (ELam v f)) ((x |-> e') ev)
--   --  x == y 
--   EApp ((x |-> e') (ELam v f)) ((x |-> e') ev)
--   EApp (ELam v f) ((x |-> e') ev)
--   ELet v ((x |-> e') ev) f
--
--   -- y `notElem` free e'
--   EApp ((x |-> e') (ELam v f)) ((x |-> e') ev)
--   EApp (ELam y ((x |-> e') e)) ((x |-> e') ev)
--   ELet y ((x |-> e') ev) ((x |-> e') e)
--
--   -- otherwise
--   EApp ((x |-> e') (ELam v f)) ((x |-> e') ev)
--   EApp (ELam y' ((x |-> e') ((y |-> EVar y') e))) ((x |-> e') ev)
--   ELet y' ((x |-> e') ev) ((x |-> e') ((y |-> EVar y') e))
...
pretty (EOp2 e1 op e2) = "(" ++ pretty e1 ++ " " ++ prettyOp2 op ++ " " ++ pretty e2 ++ ")"
  where
    ...
    prettyOp2 OLess = "<"
    prettyOp2 OAnd  = "&"
pretty (ELet v e f) = "(let "++v++" = "++pretty e++" in "++pretty f++")"
...
freshVarsT :: MonadState Int m => Int -> m [Type]
freshVarsT 0 = return []
freshVarsT n = do
  ai <-  freshVarT
  ais <- freshVarsT (n - 1)
...
freshVarT :: MonadState Int m => m Type
freshVarT = do
  i <- get
  put $ i + 1
...
s2 .# s1 = s2 +++ [(v, substType s2 t) | (v, t) <- s1 ]
...
substScheme :: TypeSub -> TypeScheme -> TypeScheme
substScheme s (TForall alphas tau) = TForall alphas (substType s' tau) where
    -- don't substitute quanitified type variables!
    s' :: TypeSub
    s' = [(v,t) | (v,t) <- s, v `notElem` alphas]

substCtx :: TypeSub -> Ctx -> Ctx
substCtx s = fmap (second (substScheme s))
...
-- INVARIANTS:
-- - returned type must already have substitution applied
-- - typing context must contain all used variables when called
infer :: MonadInfer m => Ctx -> Exp -> m (TypeSub, Type)
infer ctx (EVar x) = do
    case lookup x ctx of
        Just (TForall alphas tau) -> do
            betas <- freshVarsT (length alphas)
            let freshen = substType (zip alphas betas)
            return ([], freshen tau)
        Nothing -> throwError $ TEUnknownVar x
infer ctx (ELam x e) = do
    beta <- freshVarT
    -- arg of lambda cannot be polymorphic, instanciate monomorphic beta
    (s, tau) <- infer ((x, TForall [] beta) : ctx) e
    return (s, TFun (substType s beta) tau)
infer ctx (EApp e0 e1) = do
    (s0, tau0) <- infer ctx e0
    (s1, tau1) <- infer (substCtx s0 ctx) e1
    let tau0' = substType s1 tau0
    beta <- freshVarT
    u <- mgu1 tau0' (TFun tau1 beta)
    return (u .# s1 .# s0, substType u beta)
infer ctx (ELet x e0 e1) = do
    (s0, tau0) <- infer ctx e0
    let s0a = substCtx s0 ctx
    let sigma = gen s0a tau0
    (s1, tau1) <- infer ((x, sigma) : s0a) e1
    return (s1 .# s0, tau1)
infer _ (EBool _)  = return ([], TBool)
infer _ (EInt _)  = return ([], TInt)
infer ctx (EOp2 e0 op e1)  = do
    let (t0,t1,tr) = op2Type op
    (s0, tau0) <- infer ctx e0
    (s1, tau1) <- infer (substCtx s0 ctx) e1
    let tau0' = substType s1 tau0
    u <- mgu [(tau0', t0), (tau1, t1)]
    return (u .# s1 .# s0, substType u tr)
...
freeTs :: TypeScheme -> [Var]
freeTs (TForall alphas tau) = removeAll alphas (freeT tau)

freeCtx :: Ctx -> [Var]
freeCtx ctx = nub $ concatMap (freeTs.snd) ctx

gen :: Ctx -> Type -> TypeScheme
gen ctx tau = TForall (removeAll (freeCtx ctx) (freeT tau)) tau

run :: StateT Int (Either TypeError) a -> Either TypeError a
run mx = evalStateT mx (0 :: Int)

infer' :: Exp -> Either TypeError Type
infer' e = fmap snd.run $ infer [] e
