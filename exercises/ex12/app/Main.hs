...
pVar = do
  ...
  if x `elem` ["true", "false", "let", "in"] then
    empty
  else
    return x
...
pExp = EVar <$> pVar
   ...
   <|> ELet <$ lit '(' <* ws <* lits "let" <* ws <*> pVar <* ws <* lit '=' <* ws <*> pExp <* ws <* lits "in" <* ws <*> pExp <* ws <* lit ')'
...
example2 = parseExp "(let id = (\\x. x) in ((id id) 42))"
...
free (ELet x e1 e2) = remove x (free e2) ++ free e1
-- free (ELet x e1 e2) = free (EApp (ELam x e2) e1)
...
(x --> e') (ELet y e1 e2)                   = (x --> e') (EApp (ELam y e2) e1)
-- (x --> e') (ELet y e1 e2)                   = ELet y ((x --> e') e1) e2
...
step (ELet x e1 e2) = Just $ (x --> e1) e2
...
freshVars :: MonadState Int m => Int -> m [Var]
freshVars 0 = return []
freshVars n = do
  i <- get
  put $ i + 1
  rest <- freshVars (n-1)
  return (("alpha:" ++ show i) : rest)
...
substScheme sub (TForall bound t) = let newsub = filter (\(v,_) -> v `elem` bound) sub in
  TForall bound (substType newsub t)

(.#) :: TypeSub -> TypeSub -> TypeSub
s2 .# s1 = nub $ s2 ++ [(v, substType s2 t) | (v,t) <- s1]
...
substCtx sub = map (\(v, scheme) -> (v, substScheme sub scheme))
...
infer a (EVar x) = do
   case lookup x a of
    Just (TForall alphas tau) -> do
      betas <- freshVars $ length alphas
      return (idSub, substType [(a, TVar b) | (a,b) <- zip alphas betas] tau)
    Nothing -> throwError $ TEUnknownVar x
infer a (ELam x e) = do
  b <- freshVar
  let beta = TForall [] (TVar b)  -- beta is essentially monomorphic / simply typed!
  (s, tau) <- infer (a ++ [(x, beta)]) e
  return (s, TFun (substType s (TVar b)) tau)
infer a (EApp e0 e1) = do
  (s0, tau0) <- infer a e0
  (s1, tau1) <- infer (substCtx s0 a) e1
  b <- freshVar
  let beta = TVar b
  u <- mgu1 (substType s1 tau0) (TFun tau1 beta)
  return (u .# s1 .# s0, substType u beta)
infer a (ELet x e0 e1) = do
  (s0, tau0) <- infer a e0
  let newA = substCtx s0 a
  let sigma = gen newA tau0
  (s1, tau1) <- infer (newA ++ [(x, sigma)]) e1
  return (s1 .# s0, tau1)
infer a (EInt _) = return (idSub, TInt)
infer a (EBool _) = return (idSub, TBool)
infer a (EOp2 e1 op e2) = do
  (s1, tau1) <- infer a e1
  (s2, tau2) <- infer (substCtx s1 a) e2
  let (t1', t2', tret) = op2Type op
  u <- mgu [(tau1, t1'), (tau2, t2')]
  return (u .# s2 .# s1, tret)
...
freeScheme :: TypeScheme -> [Var]
freeScheme (TForall xs t) = [f | f <- freeT t, f `notElem` xs]
freeCtx :: Ctx -> [Var]
freeCtx = concatMap (freeScheme . snd)

gen :: Ctx -> Type -> TypeScheme
gen ctx t = TForall [f | f <- freeT t, f `notElem` freeCtx ctx] t
...
infer' e =
  case run (infer [] e) of
    ...
    Right (sub, t)
      -- | not $ null sub ->
      --     let (x, t) = head sub in
      --     throwError $ TEMismatchedType (TVar x) t
      | not $ null $ freeT t ->
