...
pExp = EVar <$> pVar
   <|> ELam <$ lit '(' <* ws <* lit '\\' <* ws <*> pVar <* ws <* lit '.' <* ws <*>
               pExp <* ws <* lit ')'
   <|> EApp <$ lit '(' <* ws <*> pExp <* ws1 <*> pExp <* ws <* lit ')'
   <|> EInt <$> pInt
   <|> EBool <$> pBool
   <|> EOp2 <$ lit '(' <* ws <*> pExp <* ws <*> pOp2 <* ws <*> pExp <* ws <* lit ')'
   <|> ELet <$ lit '(' <* ws <* lits "let" <* ws1 <*> pVar <* ws <* lit '=' <*
               ws <*> pExp <* ws1 <* lits "in" <* ws1 <*> pExp <* ws <* lit ')'
...
example = parseExp "(\\x. (let x = (1 + 2) in (x + 3)))"
...
free (ELet x e1 e2) = free e1 ++ remove x (free e2)
...
(x --> e') (ELet y e1 e2)
  | x == y                 = ELet y ((x --> e') e1) e2
  | y `notElem` free e'    = ELet y ((x --> e') e1) ((x --> e') e2)
  | otherwise              = let y' = fresh [e2, e'] in
                             ELet y' ((x --> e') e1) ((x --> e') ((y --> EVar y') e2))
-- Alternative:
-- (x --> e') (ELet y e1 e2)  = case (x --> e') (EApp (ELam y e2) e1) of
--   EApp (ELam y' e2') e1' -> ELet y' e1' e2'
--   _ -> error "this case is impossible"
...
substTypeScheme :: TypeSub -> TypeScheme -> TypeScheme
substTypeScheme s (TForall alphas t) = TForall alphas $ substType s' t where
  s' = filter (\(x, t') -> x `notElem` alphas) s

substCtx :: TypeSub -> Ctx -> Ctx
substCtx s = map (second (substTypeScheme s))
...
lookupVar :: MonadInfer m => Var -> Ctx -> m TypeScheme
lookupVar x ctx = case lookup x ctx of
  Nothing -> throwError $ TEUnknownVar x
  Just t -> return t

gen :: Ctx -> Type -> TypeScheme
gen ctx t = TForall alphas t where
  alphas = filter (\alpha -> isNothing $ lookup alpha ctx) $ freeT t

-- Composition of type substitutions.
-- Satisfies the following law:
--   substType (s1 .# s2) == substType s1 . substType s2
infixr 9 .#
(.#) :: TypeSub -> TypeSub -> TypeSub
s1 .# s2 = [ (x, substType s1 t) | (x, t) <- s2 ] ++
           [ (x, t) | (x, t) <- s1, isNothing (lookup x s2) ]
 
infer :: MonadInfer m => Ctx -> Exp -> m (TypeSub, Type)
infer ctx (EVar x) = do
  TForall alphas t <- lookupVar x ctx
  betas <- mapM (const freshVar) alphas
  return ([], substType (zip alphas (map TVar betas)) t)
infer ctx (ELam x e) = do
  beta <- freshVar
  (s, t) <- infer ((x, TForall [] (TVar beta)) : ctx) e
  return (s, TFun (substType s (TVar beta)) t)
infer ctx (EApp e1 e2) = do
  (s1, t1) <- infer ctx e1
  (s2, t2) <- infer (substCtx s1 ctx) e2
  beta <- freshVar
  s <- mgu1 (substType s2 t1) (TFun t2 (TVar beta)) 
  return (s .# s2 .# s1, substType s (TVar beta))
infer ctx (ELet x e1 e2) = do
  (s1, t1) <- infer ctx e1
  let t' = gen (substCtx s1 ctx) t1
  (s2, t2) <- infer ((x, t') : substCtx s1 ctx) e2
  return (s2 .# s1, t2)
infer ctx (EInt _) = do
  return ([], TInt)
infer ctx (EBool _) = do
  return ([], TBool)
infer ctx (EOp2 e1 op e2) = do
